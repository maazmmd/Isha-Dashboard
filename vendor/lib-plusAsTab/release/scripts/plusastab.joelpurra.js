...
(function($, namespace) {
	...
	// Private functions
	{
		function performEmulatedTabbing(isTab, isReverse, $target) {
			...
			if (isTab && $target !== undefined && $target.length !== 0) {
		...
		function isChosenTabkey(key) {
			if (key === options.key || ($.isArray(options.key) && $.inArray(key, options.key) !== -1)) {
		...
		function isEmulatedTabkey(event) {
			...
			if (!event.altKey && !event.ctrlKey && !event.metaKey && isChosenTabkey(event.which)) {
		...
		function checkEmulatedTabKeyDown(event) {
			...
			if ($target.is(disablePlusAsTab) || $target.parents(disablePlusAsTab).length > 0 || (!$target.is(enablePlusAsTab) && $target.parents(enablePlusAsTab).length === 0)) {
